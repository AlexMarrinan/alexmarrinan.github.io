...
function initWorld(){
	...
	var loader = function ( data ) {
		musicChannelId = data.channel; // save ID
	};
	PS.audioLoad("music" + currentLevelIndex, {autoplay: true, path: "audio/", volume: 0.2, loop: true, onLoad: loader});
	...
	if (speedTime > 0){
		...
		for (var xTemp = xMin; xTemp <= xMax; xTemp++){	
			for (var yTemp = yMin; yTemp <= yMax; yTemp++){
				...
				if (getColisionData(xTemp, yTemp, currentLevelIndex) == SPEED_COLOR){
					collectSpeed(xTemp, yTemp);
					setColisionData(xTemp, yTemp, EMPTY_COLOR, currentLevelIndex);
				}
				if (getColisionData(xTemp, yTemp, currentLevelIndex) == RADAR_COLOR){
					startRadar(xTemp, yTemp);
					setColisionData(xTemp, yTemp, EMPTY_COLOR, currentLevelIndex);
				}
			}
		}
